...
//	docbuild gen-nav                                        Regenerate nav-api-guide.adoc from apilist.adoc
//	docbuild gen-pdf-nav      -book B -lang L -dir D        Generate PDF nav from Antora nav file
//	docbuild inject-lang-selector -hbs PATH                 Inject language selector into header-content.hbs
//	docbuild collect-pdfs     -product P [-src S] [-dest D] Move PDFs into dest/{lang}/ structure
//	docbuild get-pdf-prefix   -product P                    Print the PDF filename prefix for a product
package main
...
import (
	...
	"github.com/uyuni-project/uyuni-docs-api/docbuild/internal/generate"
)
...
func runGenAll(repoRoot string, args []string) {
	...
	contentDir := fs.String("content-dir", defaultContentDir, "relative path to modules directory")
	_ = fs.Parse(args)

	cfg := mustLoad(filepath.Join(repoRoot, *cfgPath))

	// Generate nav first so antora.yml references an up-to-date file
	runGenNavWithConfig(repoRoot, cfg)
...
func runGenNav(repoRoot string, args []string) {
	fs := flag.NewFlagSet("gen-nav", flag.ExitOnError)
	cfgPath := fs.String("config", defaultConfig, "path to config.yml")
	_ = fs.Parse(args)
...
func runGenNavWithConfig(repoRoot string, cfg *config.Config) {
	apiListPath := filepath.Join(repoRoot, "modules", "api", "pages", "apilist.adoc")
	navOutPath := filepath.Join(repoRoot, "modules", "api", "nav-api-guide.adoc")

	if err := generate.APINav(cfg.Nav.Prepend, apiListPath, navOutPath); err != nil {
		fatalf("gen-nav: %v", err)
	}
	fmt.Printf("docbuild: regenerated %s\n", navOutPath)
}
...
func runGenPDFNav(repoRoot string, args []string) {
	...
	book := fs.String("book", "", "book name, e.g. api (required)")
...
func runCollectPDFs(repoRoot string, args []string) {
	...
	src := fs.String("src", "build", "source build directory")
	dest := fs.String("dest", "build/pdf", "destination directory")
	langsStr := fs.String("langs", "en", "space-separated list of language codes")
	_ = fs.Parse(args)
...
func usage() {
	fmt.Print(`docbuild — uyuni-docs-api build config generator

Subcommands:
  gen-all                              Generate all configs for all languages
  gen-nav                              Regenerate nav-api-guide.adoc from apilist.adoc
  ...
  gen-pdf-nav -book B -lang L -dir D       Generate nav-{book}-guide.pdf.{lang}.adoc
